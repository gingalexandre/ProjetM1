...
public class Paquet  extends UnicastRemoteObject implements serveur.modele.service.PaquetInterface, Serializable {
	...
	/**
	 * Attribut NBCHEVALIER pour le nombre de carte de type chevalier dans le
	 * paquet.
	 */
	private static int NBCHEVALIER = 14;

	/**
	 * Attribut NBINVENTION pour le nombre de carte de type invention dans le
	 * paquet.
	 */
	private static int NBINVENTION = 2;

	/**
	 * Attribut NBMONOPOLE pour le nombre de carte de type Monopole dans le
	 * paquet.
	 */
	...
	public Paquet(PaquetSauvegarde deck2) throws RemoteException {
		if (deck2.getAp() == null) {
			...
			this.ap = new ArmeePuissante();
		}
		if (deck2.getLr() == null) {
			this.lr = null;
		} else {
			this.lr = new LongueRoute();
		}
		this.deck = Fonctions.transformListCarteSauvegarde(deck2.getDeck());
	}

		...
		if (args.length > 0 && args.length < 2) {
			if (isNumeric(args[0])) {
				if (Integer.parseInt(args[0]) <= 4 && Integer.parseInt(args[0]) > 2) {
					...
					System.out.println(
							"L'argument doit être un chiffre egal a 3 ou 4.\nRedémarrez le serveur et réessayez.");
					return false;
				}
			} else {
				System.out
						.println("L'argument doit être un chiffre egal à 3 ou 4.\nRedémarrez le serveur et réessayez.");
				return false;
			}
		} else {
			System.out.println(
					"Veuillez rentrer un SEUL argument (un chiffre correspondant au nombre maximum de joueurs) lors du lancement du serveur.\nRedémarrez le serveur et réessayez.");
			return false;
		}
	}
